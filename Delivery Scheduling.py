...
def bus_driver_scheduling(minimize_drivers, max_num_drivers):
    """Optimize the delivery scheduling problem.
    ...
    for d in range(num_drivers):
        ...
        for s in range(num_shifts):
            ...
            model.Add(start_time[d] == shift[3] - setup_time).OnlyEnforceIf(source_lit)
            ...
            model.Add(end_time[d] == shift[4] + cleanup_time).OnlyEnforceIf(sink_lit)
            ...
            #    - add lower bound on end_time
            model.Add(start_time[d] <= shift[3] - setup_time).OnlyEnforceIf(
                performed[d, s]
            )
            model.Add(end_time[d] >= shift[4] + cleanup_time).OnlyEnforceIf(
                performed[d, s]
            )
        ...
        model.Add(working_times[d] == end_time[d] - start_time[d])

        if minimize_drivers:
            # Driver is not working.
            working = model.NewBoolVar("working_%i" % d)
            model.Add(start_time[d] == min_start_time).OnlyEnforceIf(working.Not())
            model.Add(end_time[d] == min_start_time).OnlyEnforceIf(working.Not())
...
def main(_):
    try:
        ...
        if num_drivers == -1:
            ...
            logger.warning("no solution found, skipping the final step")
        else:
            print("second pass: minimize the sum of working times")
            logger.info("second pass: minimize the sum of working times")
            bus_driver_scheduling(False, num_drivers)
    except Exception as exceptn:
        print(exceptn)
        logger.error("exception"%exceptn)
